## Agent Connect Framework + LangGraph

This repo contains an end-to-end example of how to expose an existing external agent **(built with LangGraph)** to **IBM Agent Connect Framework**.
...
└── acf-examples/
    ...
    └── lang-graph/
        ...
        ├── Dockerfile                # Container for the LangGraph agent server
...
4. Run the LangGraph API Server
...
6. Integrate with watsonx Orchestrate

- Use the `/v1/chat/completions` endpoint to configure your LangGraph agent as an external agent in watsonx Orchestrate.
...
1. [Example implementations of Agent Connect Framework with popular frameworks](https://connect.watson-orchestrate.ibm.com/examples#langgraph-agent-with-tool-calling)
